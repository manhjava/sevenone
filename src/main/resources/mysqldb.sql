...
grant all privileges on myproject.* to test@"%" identified by "test";
grant all privileges on myproject.* to test@localhost identified by "test";

-- Extra user/host privilege added to account for Fedora Core default hostname quirk.
grant all privileges on myproject.* to test@localhost.localdomain identified by "test";

-- You may have to explicitly define your hostname in order for things
-- to work correctly.  For example:
-- grant all privileges on myproject.* to test@host.domain.com identified by "test";
...
CREATE TABLE `Team` (
  ...
  `captain` int(50) NOT NULL,
  `joinedDate` date NOT NULL,
  `color` varchar(255) default NULL,
  `stype` int(2) NOT NULL,
  `stadium` int(50) NOT NULL,
